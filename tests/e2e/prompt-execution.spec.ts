...
test.describe('Prompt Execution Flow', () => {
    ...
    await page.route('/api/executions', async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          execution: {
            id: 'exec-1',
            result: 'Hello John! We\'re delighted to have you at TechCorp. Your friendly demeanor and expertise make you a valuable addition to our team.',
  ...
  test('should allow model selection', async ({ page }) => {
    ...
    await expect(page.getByRole('combobox', { name: /model/i })).toHaveValue('gpt-4');
  });
