...
export const useOptimizedVariableParser = (template: string) => {
  ...
  const parsedVariables = useMemo(() => {
    ...
    if (debouncedTemplate.length > 5000) {
      ...
      const advancedResult = AdvancedVariableParser.parseAdvancedTemplate(debouncedTemplate);
    ...
    if (process.env.NODE_ENV === 'development') {
      console.debug('Variable parsing performance:', {
        ...
        shouldOptimize: VariablePerformanceMonitor.shouldOptimize(parseTime, memoryUsage)
      });
    }
