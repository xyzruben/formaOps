...
export const useAccessibility = (variables: VariableDefinition[]) => {
  ...
  const announceTypeConversion = useCallback((variableName: string, fromType: string, toType: string) => {
  ...
  const announceHistoryAction = useCallback((action: 'undo' | 'redo', description: string) => {
  ...
  // Focus management
  const setFocusedVariable = useCallback((index: number) => {
    setFocusedVariableIndex(Math.max(-1, Math.min(index, variables.length - 1)));
  }, [variables.length]);
  ...
  const getVariableAriaProps = useCallback((variable: VariableDefinition, index: number) => ({
    role: 'row',
  ...
  return {
    ...
    // High contrast
    highContrastMode,
    highContrastStyles: highContrastMode ? HighContrastStyles.applyHighContrastStyles() : {}
  };
};