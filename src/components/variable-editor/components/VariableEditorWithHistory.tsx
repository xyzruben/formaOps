...
export const VariableEditorWithHistory: React.FC<VariableEditorWithHistoryProps> = ({
  ...
  const updateVariable = useCallback((index: number, updatedVariable: VariableDefinition) => {
    ...
    if (oldVariable.type !== updatedVariable.type) {
      const converted = TypeCoercionSystem.convertVariableType(oldVariable, updatedVariable.type);
      updatedVariable = { ...updatedVariable, ...converted };
      
      // Show conversion message if there's an error or warning
      if (converted.conversionResult && (converted.conversionResult.error || converted.conversionResult.warning)) {
  ...
  const handleVariableTypeChange = useCallback((index: number, newType: VariableType) => {
  ...
  return (
    ...
    >
      ...
      {/* Conversion Messages */}
      {Object.entries(conversionMessages).filter(([, message]) => message).map(([variableName, message]) => (
      ...
      {process.env.NODE_ENV === 'development' && (
        <details className="mt-4 p-3 bg-muted rounded-md text-sm">
          <summary className="cursor-pointer font-medium">History Debug</summary>
          <div className="mt-2 space-y-1">
            <div>Can Undo: {canUndo ? 'Yes' : 'No'}</div>
            <div>Can Redo: {canRedo ? 'Yes' : 'No'}</div>
            <div>Conversion Messages: {Object.keys(conversionMessages).length}</div>
          </div>
        </details>
      )}
    </div>
...
const VariableRowWithHistory: React.FC<VariableRowWithHistoryProps> = ({
  ...
  const handleTypeChangeLocal = useCallback((newType: VariableType) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            ...
            <input
              ...
              onChange={(e) => setLocalVariable(prev => ({ ...prev, name: e.target.value }))}
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => handleTypeChangeLocal(e.target.value as VariableType)}
              className="w-full p-2 border rounded-md text-sm"
            >
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              onChange={(e) => setLocalVariable(prev => ({ ...prev, description: e.target.value }))}
              className="w-full p-2 border rounded-md text-sm"
              rows={2}
            />
          </div>
          ...
          <div className="md:col-span-2 flex items-center justify-end space-x-2">
            <label className="flex items-center space-x-2 text-sm">
              <input
                ...
                onChange={(e) => setLocalVariable(prev => ({ ...prev, required: e.target.checked }))}
              />
              <span>Required</span>
            </label>
