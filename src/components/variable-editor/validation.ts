...
export const VariableDefinitionsSchema = z.array(VariableDefinitionSchema)
  .refine(variables => {
...
export const validateDefaultValue = (value: any, type: VariableType): boolean => {
  switch (type) {
    case 'string': 
...
export const extractVariablesFromTemplate = (template: string): string[] => {
  const variablePattern = /\{\{([a-zA-Z_][a-zA-Z0-9_-]*(?:\[[a-zA-Z0-9_]+\])?(?:\.[a-zA-Z_][a-zA-Z0-9_-]*)*)\}\}/g;
...
export const validateVariableName = (name: string): { isValid: boolean; error?: string } => {
  ...
  const nestedPattern = /^[a-zA-Z_][a-zA-Z0-9_-]*(?:\[[a-zA-Z0-9_]+\])?(?:\.[a-zA-Z_][a-zA-Z0-9_-]*)*$/;
  
  if (!nestedPattern.test(name)) {
    return { 
      isValid: false, 
      error: 'Variable name must be a valid identifier (letters, numbers, underscores, dots for nesting, brackets for arrays)' 
    };
  }
...
export const validateVariable = (variable: VariableDefinition): { 
  isValid: boolean; 
  errors: string[] 
} => {
  const errors: string[] = [];
